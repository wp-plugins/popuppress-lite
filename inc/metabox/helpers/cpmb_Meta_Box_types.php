...
class cpmb_Meta_Box_types {
	 ...
	 * @var cpmb_Meta_Box_types object
	 */
	 ...
	 * @since  1.0.0
	 * @return cpmb_Meta_Box_types A single instance of this class.
	 */
	...
	private static function desc( $desc, $paragraph = false ) {
		...
		return "\n<$tag class=\"cpmb_metabox_description\">$desc</$tag>\n";
	}
	...
	private static function repeat_table_open( $class = '' ) {
		echo self::desc( self::$field['desc'] ), '<table id="', self::$field['id'], '_repeat" class="cpmb-repeat-table ', $class ,'"><tbody>';
	}
	...
	private static function repeat_table_close() {
		echo '</tbody></table><p class="add-row"><a data-selector="', self::$field['id'] ,'_repeat" class="add-row-button button" href="#">'. __( 'Add Row', 'cpmb' ) .'</a></p>';
	}
	...
	private static function repeat_cell( $input ) {
		return '<td>'. $input .'</td><td class="remove-row"><a class="button remove-row-button" href="#">'. __( 'Remove', 'cpmb' ) .'</a></td>';
	}
	...
	public static function text_small( $field, $meta ) {
		echo '<input class="cpmb_text_small" type="text" name="', $field['id'], '" id="', $field['id'], '" value="', '' !== $meta ? $meta : $field['std'], '" />', self::desc( $field['desc'] );
	}

	public static function text_small_repeat( $field, $meta ) {
		self::repeat_text_field( $field, $meta, 'cpmb_text_small' );
	}

	public static function text_medium( $field, $meta ) {
		echo '<input class="cpmb_text_medium" type="text" name="', $field['id'], '" id="', $field['id'], '" value="', '' !== $meta ? $meta : $field['std'], '" />', self::desc( $field['desc'] );
	}

	public static function text_medium_repeat( $field, $meta ) {
		self::repeat_text_field( $field, $meta, 'cpmb_text_medium' );
	}

	public static function text_email( $field, $meta ) {
		echo '<input class="cpmb_text_email cpmb_text_medium" type="email" name="', $field['id'], '" id="', $field['id'], '" value="', '' !== $meta ? $meta : $field['std'], '" />', self::desc( $field['desc'], true );
	}

	public static function text_email_repeat( $field, $meta ) {
		self::repeat_text_field( $field, $meta, 'cpmb_text_email cpmb_text_medium', 'email' );
	}
	...
	public static function text_url( $field, $meta ) {
		...
		echo '<input class="cpmb_text_url cpmb_text_medium" type="text" name="', $field['id'], '" id="', $field['id'], '" value="', $val, '" />', self::desc( $field['desc'], true );
	}
	...
	public static function text_url_repeat( $field, $meta ) {
		...
		$protocols = isset( $field['protocols'] ) ? (array) $field['protocols'] : null;
		$val = $val ? esc_url( $val, $protocols ) : '';
		self::repeat_text_field( $field, $val, 'cpmb_text_url cpmb_text_medium' );
	}

	public static function text_date( $field, $meta ) {
		echo '<input class="cpmb_text_small cpmb_datepicker" type="text" name="', $field['id'], '" id="', $field['id'], '" value="', '' !== $meta ? $meta : $field['std'], '" />', self::desc( $field['desc'] );
	}

	public static function text_date_timestamp( $field, $meta ) {
		echo '<input class="cpmb_text_small cpmb_datepicker" type="text" name="', $field['id'], '" id="', $field['id'], '" value="', '' !== $meta ? date( 'm\/d\/Y', $meta ) : $field['std'], '" />', self::desc( $field['desc'] );
	}

	public static function text_datetime_timestamp( $field, $meta, $object_id ) {

		// This will be used if there is a select_timezone set for this field
		$tz_offset = cpmb_Meta_Box::field_timezone_offset( $object_id );
		if ( !empty( $tz_offset ) ) {
			$meta -= $tz_offset;
		}
		...
		echo '<input class="cpmb_timepicker text_time" type="text" name="', $field['id'], '[time]" id="', $field['id'], '_time" value="', '' !== $meta ? date( 'h:i A', $meta ) : $field['std'], '" />', self::desc( $field['desc'] );
	}
	...
	public static function text_datetime_timestamp_timezone( $field, $meta ) {
		...
		echo '<input class="cpmb_text_small cpmb_datepicker" type="text" name="', $field['id'], '[date]" id="', $field['id'], '_date" value="', '' !== $meta ? date( 'm\/d\/Y', $meta ) : $field['std'], '" />';
		echo '<input class="cpmb_timepicker text_time" type="text" name="', $field['id'], '[time]" id="', $field['id'], '_time" value="', '' !== $meta ? date( 'h:i A', $meta ) : $field['std'], '" />';
	...
	public static function text_time( $field, $meta ) {
		echo '<input class="cpmb_timepicker text_time" type="text" name="', $field['id'], '" id="', $field['id'], '" value="', '' !== $meta ? $meta : $field['std'], '" />', self::desc( $field['desc'] );
	}
	...
	public static function text_money( $field, $meta ) {
		echo ! empty( $field['before'] ) ? '' : '$', ' <input class="cpmb_text_money" type="text" name="', $field['id'], '" id="', $field['id'], '" value="', '' !== $meta ? $meta : $field['std'], '" />', self::desc( $field['desc'] );
	}

	public static function text_money_repeat( $field, $meta ) {
		self::repeat_text_field( $field, $meta, 'cpmb_text_money' );
	}
	...
	public static function colorpicker( $field, $meta ) {
		...
		elseif ( ! preg_match( '/^#' . $hex_color . '/i', $meta ) ) // Value doesn't match #123abc, so sanitize to just #.
			$meta = "#";
		echo '<input class="cpmb_colorpicker cpmb_text_small" type="text" name="', $field['id'], '" id="', $field['id'], '" value="', $meta, '" />', self::desc( $field['desc'] );
	}
	...
	public static function textarea_code( $field, $meta ) {
		echo '<textarea name="', $field['id'], '" id="', $field['id'], '" cols="60" rows="10" class="cpmb_textarea_code">', '' !== $meta ? $meta : $field['std'], '</textarea>', self::desc( $field['desc'], true );
	}
	...
	public static function radio_inline( $field, $meta ) {
		if( empty( $meta ) && !empty( $field['std'] ) ) $meta = $field['std'];
		echo '<ul class="cpmb_radio_inline">';
		$i = 1;
		foreach ($field['options'] as $option) {
			echo '<li class="cpmb_radio_inline_option"><input type="radio" name="', $field['id'], '" id="', $field['id'], $i, '" value="', $option['value'], '"', checked( $meta == $option['value'] ), ' /> <label for="', $field['id'], $i, '">', $option['name'], '</label></li>';
			$i++;
		}
	...
	public static function radio( $field, $meta ) {
		...
		$i = 1;
		foreach ($field['options'] as $option) {
			echo '<li class="cpmb_radio_option"><input type="radio" name="', $field['id'], '" id="', $field['id'], $i,'" value="', $option['value'], '" ', checked( $meta == $option['value'] ), ' /> <label for="', $field['id'], $i, '">', $option['name'].'</label></li>';
			$i++;
		}
		echo '</ul>', self::desc( $field['desc'], true );
	}
	...
	public static function title( $field, $meta, $object_id, $object_type ) {
		...
		echo '<'. $tag .' class="cpmb_metabox_title">', $field['name'], '</'. $tag .'>', self::desc( $field['desc'], true );
	}
	...
	public static function taxonomy_radio( $field, $meta, $object_id ) {
		...
		foreach ( $terms as $term ) {
			...
			echo '<li class="cpmb_radio_inline_option"><input type="radio" name="', $field['id'], '" id="', $field['id'], $i,'" value="'. $term->slug . '" ', checked( $checked ), ' /> <label for="', $field['id'], $i, '">' . $term->name . '</label></li>';
			$i++;
		}
		echo '</ul>', self::desc( $field['desc'], true );
	}
	...
	public static function file_list( $field, $meta, $object_id ) {
		...
		echo '<input class="cpmb_upload_file cpmb_upload_list" type="hidden" size="45" id="', $field['id'], '" name="', $field['id'], '" value="', $meta, '" />';
		echo '<input class="cpmb_upload_button button cpmb_upload_list" type="button" value="'. __( 'Add or Upload File', 'cpmb' ) .'" />', self::desc( $field['desc'], true );

		echo '<ul id="', $field['id'], '_status" class="cpmb_media_status attach_list">';
			...
			foreach ( $meta as $id => $fullurl ) {
				if ( self::is_valid_img_ext( $fullurl ) ) {
					...
					'<li class="img_status">',
						...
						'<p><a href="#" class="cpmb_remove_file_button">'. __( 'Remove Image', 'cpmb' ) .'</a></p>
					...
					echo
					'<li>',
						__( 'File:', 'cpmb' ), ' <strong>', $title, '</strong>&nbsp;&nbsp;&nbsp; (<a href="', $fullurl, '" target="_blank" rel="external">'. __( 'Download', 'cpmb' ) .'</a> / <a href="#" class="cpmb_remove_file_button">'. __( 'Remove', 'cpmb' ) .'</a>)
						<input type="hidden" id="filelist-', $id ,'" name="', $field['id'] ,'[', $id ,']" value="', $fullurl ,'" />
					</li>';
				}
			}
	...
	public static function file( $field, $meta, $object_id, $object_type ) {
		...
		echo '<input class="cpmb_upload_file_id" type="hidden" id="', $_id_name, '" name="', $_id_name, '" value="', $_id_meta, '" />',
		self::desc( $field['desc'], true ),
		'<div id="', $field['id'], '_status" class="cpmb_media_status">';
			if ( ! empty( $meta ) ) {
	...
	public static function oembed( $field, $meta, $object_id, $object_type ) {
		...
		echo '<div id="', $field['id'], '_status" class="cpmb_media_status ui-helper-clearfix embed_wrap">';
			...
			if ( $meta != '' )
				echo cpmb_Meta_Box_ajax::get_oembed( $meta, $object_id, array(
	...
	/**
	 * Default fallback. Allows rendering fields via "cpmb_render_$name" hook
	 * @since  1.0.0
	...
	public function __call( $name, $arguments ) {
		...
		do_action( "cpmb_render_$name", $field, $meta, $object_id, $object_type );
	}
